...
################################################################################
        ...
        Sigmav = sigma2[it] * np.eye(N)
        Sigmaw = sigma2[it] * np.eye(M)

        xt, yt = particles.createLinearTrajectoryNongaussian(T, A, C,
        ...
        with open(os.path.join(dataSplitDir,'dataG%02d.pkl' % rlztn), 'wb') as thisFile:
            pickle.dump(varsToSave, thisFile)
